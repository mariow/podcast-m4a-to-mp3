...
func main() {
	...
	for _, sec := range secs {
		if strings.HasPrefix(sec, "podcast ") {
			podcast_name := strings.ToLower(strings.TrimPrefix(sec, "podcast "))
			...
			if url != "" {
				...
				for s.Scan() {
					...
					if len(matchs) > 1 {
						...
						if !fileExists(outf_fname) {
							...
							err = transcodeFile(tmp_download.Name(), outf_fname)
							if err != nil {
				...
				// save updated rss
				copyFile(tmp_outfile.Name(), output_path+podcast_name+".rss")
			}
		}
	}
}
